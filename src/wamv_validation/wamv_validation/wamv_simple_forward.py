...
class WamvSimpleForward(Node):
    def __init__(self, thrust=100.0, on_time=16.25, off_time=10.0):
        super().__init__('wamv_simple_forward')
        ...
        self.get_logger().info("Forward thrust node started. ON/OFF cycle begins immediately.")
    ...
    def publish_thrust(self):
        ...
        # --- PUBLISH THRUST ---
        thrust_cmd = self.thrust if self.state == "ON" else 0.0
        ...
        self.left_pub.publish(Float64(data=thrust_cmd))
        self.right_pub.publish(Float64(data=thrust_cmd))

def main(args=None):
    rclpy.init(args=args)
    node = WamvSimpleForward(thrust=100.0, on_time=16.25, off_time=10.0)
    rclpy.spin(node)
    rclpy.shutdown()
