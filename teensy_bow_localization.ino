...
// ROS callbacks
// -------------------------
void rtcm_callback(const void *msgin)
{
  ...
  // Push moving-base RTCM corrections into the bow GNSS.
...
void relposnedCallback(UBX_NAV_RELPOSNED_data_t *ubxDataStruct)
{
  ...
  stamp_now(&msg_heading.header, gps_frame);

  const double heading_rad = (ubxDataStruct->relPosHeading / 100000.0) * (PI / 180.0);
  ...
  for (int i = 0; i < 36; i++)
    msg_heading.pose.covariance[i] = 0.0;
...
bool create_entities()
{
  ...
  RCCHECK(rclc_node_init_default(&node, "localization_bow_node", "", &support));
...
void configure_bow_gnss()
{
  ...
  myGNSS.addCfgValset(UBLOX_CFG_MSGOUT_UBX_NAV_RELPOSNED_UART1, 1);
